...
export default function StudentAuthGate({
  ...
  useEffect(() => {
    ...
    // CRITICAL: We must manually handle the OAuth callback because this is a custom flow.
    // Without handleRedirectCallback and setActive, the Clerk session is never created!
    // The ref ensures this only runs exactly once per component mount.
    if (sessionStorage.getItem("clerk-oauth-intent") === "student-auth" && !hasProcessedCallback.current) {
      hasProcessedCallback.current = true;
      clerk.handleRedirectCallback()
        .then(() => {
          const sid = clerk.client.signIn?.createdSessionId || clerk.client.signUp?.createdSessionId;
          if (sid) {
            clerk.setActive({ session: sid }).catch(console.error);
          }
        })
        .catch(() => {
          // Ignore errors here as this might fail if no OAuth params exist in URL
        });
    }
  ...
  const handleGoogleAuth = async () => {
    ...
    // Tell App.tsx to restore the student-auth view after the OAuth redirect reloads the page
    sessionStorage.setItem("clerk-oauth-intent", "student-auth");
  ...
  // This prevents UI flicker and stops Clerk's default <SignIn /> from
  // briefly appearing alongside our custom form.
  if (!userLoaded) {
    return (
      <div className="min-h-screen landing-page-black" aria-hidden="true" />
    );
  }
  ...
  return (
    <div className="min-h-screen landing-page-black flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.35 }} className="w-full max-w-md relative z-10">
        <button onClick={handleBack} className="flex items-center gap-1.5 text-sm text-white/70 hover:text-white mb-8 transition-colors">
        ...
        <Card className="p-8 !bg-black border border-white/10 shadow-[0_32px_80px_rgba(0,0,0,0.35)] relative overflow-hidden group premium-border-card">
          ...
          <button onClick={handleGoogleAuth} disabled={loading || checkingBackend}
            ...
            <span className="flex-1 text-center text-sm font-semibold text-white">
              {loading ? "Signing in…" : checkingBackend ? "Verifying…" : "Sign in with Google"}
            </span>
          </button>
